...
@interface PhotoSignViewController() <DBRestClientDelegate>

@property NSString *privKeyPIN;
...
-(void)showAlertPassword {
	...
	// UIAlertView *myAlertView = [[UIAlertView alloc] initWithTitle:@"Password"
    //                    message:@"\n\n" delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"OK", nil];
	
    UIAlertView * myAlertView =  [[UIAlertView alloc]
                    initWithTitle:@"Insert PIN"
...
-(Boolean) checkLoadKey {
    ...
    privKeyPath = [[NSBundle mainBundle] pathForResource:@"mykey" ofType:@"key"];

	//carico la chiave privata nella struttura RSA 
	RSA *privKey = [rsa loadPrivateKey:privKeyPath Key:self.privKeyPIN];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    PhotoSignAppDelegate *appDelegate = (PhotoSignAppDelegate *) [[UIApplication sharedApplication] delegate];
	 if (alertView.tag == 0) {
		...
		if (buttonIndex == 1) {
            self.privKeyPIN = [[alertView textFieldAtIndex:0] text];
            NSLog(@"Entered: %@", self.privKeyPIN);
...
-(void) uploadFileOnMainThread: (NSString *)imgPath {
    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    [self performSelectorOnMainThread:@selector(uploadFile:) withObject: imgPath waitUntilDone:YES];
    [pool release];
}
...
-(void)imageProcessing:(NSData *)imageData {
    ...
    double kk =overlayView.locationPoint.coordinate.latitude;
	locArrayLat = [self createLocArray:kk];
	...
	if (overlayView.locationPoint.coordinate.latitude <0.0){
		...
		refLat =@"N";   // il metodo tagValue ritorna la stringa con un \0 alla fine $&%$&$%$
	}
	...
	[exifData addTagValue: @"eTuitus s.r.l." forKey:[NSNumber numberWithInt:EXIF_Make]];
	[exifData addTagValue: @"Spin-Off dell'Universita' di Salerno" forKey:[NSNumber numberWithInt:EXIF_Copyright]];
	[exifData addTagValue: @"Iron Track 3.0" forKey:[NSNumber numberWithInt:EXIF_Software]];
    ...
    certPath = [[NSBundle mainBundle] pathForResource:@"mycert" ofType:@"crt"];
	...
	if ((privKey == nil) || (cert == nil)) {
        ...
        NSData *dataForTSA = [hashSec SHA256: taggedDataForTSA];
        
        NSLog(@"Image SHA256:%@", [commonSec base64EncodedStringWithData:dataForTSA]);
		...
		NSData *dataRequest = [tspSec CreateRequestWithHash:dataForTSA HashName:@"SHA256" Policy:@"" nonce:0 CertRequired:YES];
		NSString *postLength = [NSString stringWithFormat:@"%d", [dataRequest length]];
		NSMutableURLRequest *request = [[NSMutableURLRequest alloc] init ];
        // [request setURL:[NSURL URLWithString:@"https://clepsydre.edelweb.fr/dvcs/service-tsp"]];
        [request setURL:[NSURL URLWithString:@"https://marte.infocert.it/cdie/HttpService"]];
        
		[request setHTTPMethod:@"POST"];
		[request setValue:postLength forHTTPHeaderField:@"Content-Length"];
        [request setValue:@"application/timestamp-reply" forHTTPHeaderField:@"Accept"];
        [request setValue:@"Basic eXllOTEzODppbmZvY2VydDE=" forHTTPHeaderField:@"Authorization"];
        //
		...
		if ((resp == tspGranted) || (resp == tspGrantedWithModification)) {
			...
			if (pkcs7 == nil) {
				...
				if  (pkcs7Data == nil) {
					...
					//Path locale dell'immagine
                    NSString *filename = [NSString stringWithFormat:@"%@%@%@", @"IMG", dateStrPath, @".jpg"];
                    
                    NSString *imgPath = [[appDelegate imagePath] stringByAppendingPathComponent: filename];
                    ...
                    // upload to dropbox in main thread
                    [self changeLabelprogressTextOnMainThread:@"Uploading image to dropbox ..."];
                    [self uploadFileOnMainThread:imgPath];
...
-(void) sendPECAttachment: (NSString *) filePath {
	...
	[picker setSubject:@"Immagine firmata e geolocalizzata"];
	
	// Set up recipients
	NSArray *toRecipients  = [NSArray arrayWithObject:@"etuitus@gmail.com"];
	
	NSArray *ccRecipients  = [NSArray arrayWithObjects:@"giuseppe.cattaneo@gmail.com", nil];
...
// solo queste funzione richiede il framework CFNetwork altrimenti eliminare ed il package SMTPSender
// invio foto via EMail ... risultato: "molto lento", credo dipenda dall'implementazione SMTP
...
-(void) uploadFile: (NSString *) filePath {
    ...
    [self.restClient uploadFile:[filePath lastPathComponent]
                     toPath:destDir withParentRev:nil fromPath:filePath];
