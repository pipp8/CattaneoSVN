...
-(void)onTimer:(NSTimer *)timer {
	if (([[DBSession sharedSession] isLinked]) && fileCount == 0) {
		...
		[self.restClient loadMetadata:@"/Apps/MyPhotoSign-Verify/Images" withHash:nil];
	}
}
...
- (void)restClient:(DBRestClient*)client loadedMetadata:(DBMetadata*)metadata {
    ...
    NSArray* validExtensions = [NSArray arrayWithObjects:@"jpg", nil];
    ...
    for (DBMetadata* child in metadata.contents) {
		...
		if (!child.isDirectory && [validExtensions indexOfObject:extension] != NSNotFound) {
			...
			fileNameAtPath = [NSString stringWithFormat:@"%@/%@", [appDelegate imagePath],fileName];
	...
	for (NSInteger i= 0; i< [directoryContent count]; i++) {
		...
		if (!trovato) {
		    go = NO;
		    [fileManager removeItemAtPath:[NSString stringWithFormat:@"%@/%@",[appDelegate imagePath],file] error:nil];
...
- (void)restClient:(DBRestClient*)client loadMetadataFailedWithError:(NSError*)error {
	...
	[self.restClient createFolder:@"/Apps/MyPhotoSign-Verify/Images"];
	fileCount = 0;
}
...
- (void)restClient:(DBRestClient *)client deletePathFailedWithError:(NSError *)error {
	...
	[[[[UIAlertView alloc] 
       initWithTitle:@"Errore" message:@"Impossibile cancellare l'immagine."
...
- (void)restClient:(DBRestClient *)client deletedPath:(NSString *)path {
	...
	[fileManager removeItemAtPath:[NSString stringWithFormat:@"%@/%@",[appDelegate imagePath],fileName] error:nil];
    
	NSString *pngPath = [NSString stringWithFormat:@"%@/%@.png",[appDelegate imagePath],[fileName stringByDeletingPathExtension]];
...
- (void)restClient:(DBRestClient*)client loadFiledWithError:(NSError*)error {
	...
	[[[[UIAlertView alloc] 
       initWithTitle:@"Errore" message:@"Impossibile scaricare l'immagine."
       delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil]
      autorelease]
     show];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
	...
	NSMutableArray *array = [[NSMutableArray alloc] initWithArray:[[fileManager contentsOfDirectoryAtPath:[appDelegate imagePath] error:nil] filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"self ENDSWITH '.jpg'"] ]]; 
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
   MyPhotoVerifyAppDelegate *appDelegate = (MyPhotoVerifyAppDelegate *) [[UIApplication sharedApplication] delegate];
	...
	UIImage *image = [[UIImage alloc] initWithContentsOfFile:[NSString stringWithFormat:@"%@/%@.png",[appDelegate imagePath],[[directoryContent objectAtIndex:[indexPath row]] stringByDeletingPathExtension  ]]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	[imageViewController setImageFile:[directoryContent objectAtIndex:[indexPath row]]];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (editingStyle == UITableViewCellEditingStyleDelete) {
		fileCount++;
		[self.restClient deletePath:[NSString stringWithFormat:@"%@/%@",@"/Apps/MyPhotoSign-Verify/Images" , [directoryContent objectAtIndex:[indexPath row]]]];
