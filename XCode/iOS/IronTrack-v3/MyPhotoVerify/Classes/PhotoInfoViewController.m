...
-(void) verifyPhoto {
    ...
    value = [exifData tagValue:[NSNumber numberWithInt:EXIF_GPSLatitudeRef]];
    NSString *latRef = [NSString stringWithFormat:@"%c", [value characterAtIndex:0]];
//    NSString *latRef = [NSString stringWithCString:(char *) value encoding:NSUTF8StringEncoding];

    EXFGPSLoc *gpsLocLong = [exifData tagValue:[NSNumber numberWithInt:EXIF_GPSLongitude]];

    value = [exifData tagValue:[NSNumber numberWithInt:EXIF_GPSLongitudeRef]];
    NSString *longRef = [NSString stringWithFormat:@"%c", [value characterAtIndex:0]];
//    NSString *longRef = [NSString stringWithCString:(char *) value encoding:NSUTF8StringEncoding];
	...
	lblGPSCoord.text = [NSString stringWithFormat:@"%@ %@ %@ %@",
                        latRef, [self GPSLoc2Str:gpsLocLat],
                        longRef, [self GPSLoc2Str:gpsLocLong]];
	...
	if (((respTSP == tspGranted) || (respTSP == tspGrantedWithModification)) && ([b64Sign length] > 0)) {
		...
		if (pkcs7 != nil) {
			...
			NSData *dataToVerifyForTSP = [hashSec SHA256:taggedDataForTSP];
			[taggedDataForTSP release];
            
            
            NSLog(@"Image SHA256:%@", [commonSec base64EncodedStringWithData:dataToVerifyForTSP]);

            X509 *CACert = [x509Sec loadCertificateWithPath:[NSString stringWithFormat:@"%@/%@",
                                                                        [appDelegate certPath], @"CA_Infocert-ca-tsa2.crt"]];
            X509 *TSACert = [x509Sec loadCertificateWithPath:[NSString stringWithFormat:@"%@/%@", [appDelegate certPath],
                                                              @"ic_tsa.crt"]];

			//Verifico La risposta dalla TSA
			
			NSData *dataRequest = [tspSec CreateRequestWithHash:dataToVerifyForTSP HashName:@"SHA256" Policy:@"" nonce:0 CertRequired:YES];
            
            
            verify = [tspSec verifySignatureWithResponse:dataTSP Request:dataRequest Cert:TSACert CACert:CACert];
...
//            if (certNum >= 0) {
            if (verify == YES) {
				...
				lblTSASigner.text = [x509Sec getCertSubject:TSACert Parameter:@"CN"];
...
//					[x509Sec FreeX509Struct:cert];
            ...
            [x509Sec FreeX509Struct:CACert];
            [x509Sec FreeX509Struct:TSACert];
...
-(double) fraction2Rational: (EXFraction *) p
{
    return ((double) p.numerator / (double) p.denominator);
}


-(NSString *) GPSLoc2Str: (EXFGPSLoc *) location
{
    NSString * ret = [NSString stringWithFormat:@"%ld\xC2\xB0 %ld' %ld\x22",   // simboli unicode x gradi, primi e secondi
                      location.degrees.numerator / location.degrees.denominator,
                      location.minutes.numerator / location.minutes.denominator,
                      location.seconds.numerator / location.seconds.denominator];
