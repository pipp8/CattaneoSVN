...
- (void) test2: (id) callBackObj {
    ...
    self.target = callBackObj;
    NSString *key  = @"1234567890123";
    
    // Get the HEX in a Byte[]
    // NSData * msg = [TuitusOTP hexStr2Bytes: smsKey];
    NSData *msg =[key dataUsingEncoding:NSUTF8StringEncoding];
    ...
    self.remoteSeed = [TuitusOTP bytes2HexStr: binSeed];
...
- (void) fineStep1 {
    ...
    [[self.target getSpinner] stopAnimating];
...
//    [self.infoAlert  show];
    ...
    // torna al viewcontroller di partenza che poi avvierà la navigazione alla prossima view
...
- (void) fineStep2 {
    ...
    // ritorna alla seconda view
    [self.target restartView: self.remoteSeed result: !_serverFailure];
...
- (void) openSession: (NSString *) user withPassword: (NSString *) password
    ...
    NSCharacterSet *URLCombinedCharacterSet = [[NSCharacterSet characterSetWithCharactersInString:@" \"#%/:<>?@[\\]^`{|}&+"] invertedSet]; // aggiunto il +
    
    NSString *encUsername = [user stringByAddingPercentEncodingWithAllowedCharacters: URLCombinedCharacterSet];
    NSString *encPassword = [password stringByAddingPercentEncodingWithAllowedCharacters: URLCombinedCharacterSet];

    NSString *bodyData = [NSString stringWithFormat: @"login=%@&password=%@", encUsername, encPassword];
    
    // NSData *requestBodyData = [bodyData dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES]; // non funziona
...
- (void) requestSMS {
    ...
    self.progressStatus = OTPSMSRequest; // 2
    ...
    [request addValue:[self.cookiesL1Dict valueForKey: @"csrf-token"] forHTTPHeaderField:@"X-CSRFToken"];
...
- (void) sendSMS: (NSString *) smsOTP target: (id) callBackObj {
    ...
    self.progressStatus = OTPVerifyRequest; // 4
...
- (void) getSeed {
    ...
    self.progressStatus = SeedRequest; // 5
    
//    NSString * srvPath = @"API/lcsi-selfcare-api/identita/app/seed";
    NSString * srvPath = @"relm-selfcare-api/identita/app/seed";
    NSString * url = [NSString stringWithFormat: @"%@/%@", self.provisioningSrv, srvPath];
...
- (void) sendConfirm {
   ...
   forHTTPHeaderField:@"X-CSRFToken"];
    ...
    time_t minutes = [[NSDate date] timeIntervalSince1970] / clockRound;
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    if (code >= 400) {
        ...
        if (self.progressStatus == LoginReq) {
            ...
            self.cookiesL1Dict = [self parseCockies: self.cookieL1Str];
        ...
        else if (self.progressStatus == OTPSMSRequest) {
        
            // recupera l'eventual prefisso dell'OTP inserito nell'SMS
            self.OTPPrefix = [fields valueForKey:@"X-PRE-OTP"]; // C'e' sempre ???
            if (!self.OTPPrefix)
                self.OTPPrefix = @"";
            
            NSLog(@"OTP Prefix set to: %@\n", self.OTPPrefix);
        }
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
    ...
    NSString * msg = [NSString stringWithFormat:NSLocalizedString(@"CONNECTION-ERROR", nil),
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    switch ((int) self.progressStatus) {
        ...
        case OTPSMSRequest:     // [self sendSMS] continua con il bottone;
            ...
            [self fineStep1]; // anche se fallisce chiude lo step1
            break;
            
        case OTPVerifyRequest:
            if (!_serverFailure) {
                // solo se l'OTP viene verificaato
                [self getSeed]; //procede con la richiesta del seed
            }
            else {
                [self fineStep2]; // interrompe per restare nella view e reinserire l'OTP
            }
            break;
        ...
        case SeedRequest:
            ...
            if (!_serverFailure) {
                ...
                NSLog(@"Seed: <%@> lenght: %lu", _remoteSeed, (unsigned long)[_remoteSeed length]);
                [self sendConfirm];
            }
            else {
                [self fineStep2]; // interrompe
            }
            break;
...
- (void) initErrorMessages
{
    ...
    [_errorMessages setValue: NSLocalizedString(@"AUTHFAIL-ERR", nil) forKey: strCode];
    ...
    [_errorMessages setValue: NSLocalizedString(@"FIRSTOTPFAILED-ERR", nil) forKey:strCode];
