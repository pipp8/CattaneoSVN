...
#
# Modified to print Quantization Table by Cattaneo
# $Id: JPEGDigest.pm 182 2011-09-13 14:48:45Z cattaneo $
# $LastChangedDate: 2011-09-13 16:48:45 +0200 (Tue, 13 Sep 2011) $
#
#------------------------------------------------------------------------------
...
sub Calculate($$$)
{
    ...
    foreach $dat (@$dqtList) {
	    ...
	    $t = 0; $p = 1;
        while( $t < 2) {
	        if (($t == 0) && ($TableType == 1) && (length($dat) > 64)) {
	        	print("Luminance DQT:");
	        }
        	elsif ($t == 1 || $TableType == 2) {
	        	print("Chrominance DQT:");
        	}

        	for($k = 0; $k < 64; $k++) {
        		$table[$jpeg_natural_order[$k]] = ord(substr($dat, $k+$p, 1));
        	}
        	for($k = 0; $k < 64; $k++) {
				print("\n") if (($k % 8 == 0));
				printf( "%3d ", $table[$k]);
	    	...
	    	$TableType++;
	    	if (length($dat) > 128) {
		    	$p = 66;	# start index of the next table
		    	$t++;
	    	}
	    	else {
	    		last;
