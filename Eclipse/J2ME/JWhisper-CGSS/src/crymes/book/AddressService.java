...
//	private static CryptoSMSMidlet _midlet;
	...
	RecordStoreException {
		...
		CGSSLogger.getInstance().write("Save the outgoing message ("+m.getSender()+") in the store");
	}
...
//					+ url);
		...
		a.setPublicKey(data);
		CGSSLogger.getInstance().write("Save the public key for "+url);
		updateAdress(a);
	}
	...
	RecordStoreException {
		...
		a.setPublicKey(data);
		CGSSLogger.getInstance().write("Delete old public key for "+url);
		updateAdress(a);
	}
...
//					+ url);
		...
		CGSSLogger.getInstance().write("Make valid the public key for "+url);
		updateAdress(a);
	}
	/**
	 * @param a
