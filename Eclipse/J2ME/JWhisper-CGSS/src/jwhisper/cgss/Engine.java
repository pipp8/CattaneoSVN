...
import jwhisper.cgss.modules.connection.CIncomingMessageListener;
import jwhisper.cgss.modules.connection.ISmsMessageListener;
import jwhisper.cgss.modules.connection.MessageReader;
import jwhisper.cgss.modules.connection.MessageSender;
import jwhisper.cgss.modules.connection.Receiver;
import jwhisper.cgss.modules.handlers.GSMClient;
import jwhisper.cgss.modules.handlers.IGenericClient;
import jwhisper.cgss.modules.handlers.WtkClient;
...
public class Engine  implements ISmsMessageListener{
	...
	public void sendSms (jwhisper.cgss.modules.wma.BinaryMessage m){
	    ...
	    try {
	    	...
	    	CGSSLogger.getInstance().write("Payload size: " + m.getPayloadData().length);
			sent = gClient.send(m);
		} catch (IOException e) {
			CGSSLogger.getInstance().write("Exception sending sms.");
		}
		
		if (sent){			
			try {
				  
				CMessage m2 = CMessage.createFromTransport( MessageUtils.JWhisperBinaryMessage2J2MEBinaryMessage(m));
				AddressService.saveOutgoingMessage(m2);
				addRowToMessageList(m2);
			} catch (Exception e) {
				e.printStackTrace();
				CGSSLogger.getInstance().write("Cannot show the sms sent in the table");
				//e.printStackTrace();
			}
	...
	public void notifyIncomingMessage(jwhisper.cgss.modules.wma.BinaryMessage msg, boolean isEmulator)  {
...
//	    }
	...
	public void initAddressService(String pass) throws Exception {
		...
		_settings = (SettingsA2P) AddressService.getSetting();
		if (_settings!=null){
	...
	public void setSettings(SettingsA2P s) {
		this._settings = s;
	}
			...
			Runnable p = new Runnable() {
				public void run(){
					...
					try {
						...
						SettingsA2P s= (SettingsA2P) AddressService.saveSettings(_algorithm, n, gsm, k, gsm);
						setSettings(s);
						...
						CGSSLogger.getInstance().write("Error while starting thread: "  + "\n"+ e.getMessage());					
					} finally{
		//				stop();
					}
				}
...
//								+ pers[i].getConns()[j].hasKey(), listImage(pers[i].getConns()[j].hasKey()));
	...
	public void addEntry(String name, String address) {
		try {
			...
			CGSSLogger.getInstance().write("Added " + name + " to address book");
		} catch (Exception e) {
			CGSSLogger.getInstance().write("Error: cannot add " + name + "to address book");
		}
	...
	public void deleteEntry(int row) {
		...
		try {
			...
			CGSSLogger.getInstance().write("Deleted address entry of " + name);
		} catch (Exception e) {
			CGSSLogger.getInstance().write("Error: cannot delete " + name + "from address book");
		}
		//MAU_mainFrame.updateRecipientList();
		_mainFrame.updateAddressListTable();
	...
	public void editEntry(int row, String name, String addressStr) {
		...
		try {
			...
			CGSSLogger.getInstance().write("Updated address entry of " + name);
		} catch (Exception e) {
			CGSSLogger.getInstance().write("Error: cannot update " + name + "in address book");
		}
		//MAU_mainFrame.updateRecipientList();
		_mainFrame.updateAddressListTable();
	...
	public void PK_Register(CMessage m) {
		...
		if (m.getType() == MessageType.PK_REGISTER_REQUEST ) {
			...
			try {
				...
				CGSSLogger.getInstance().write("key (not verified) added to "+m.getSender());
			} catch (Exception e) {
				CGSSLogger.getInstance().write("key added error to "+m.getSender());
				return;
			}
			...
			try { 
				...
				CGSSLogger.getInstance().write(e.getMessage());				
			}
	...
	public void completeRegistration(CMessage m){
		if (m.getType() == MessageType.PK_CHALLENGE_RESPONSE ) {
			...
			try {
				...
				CGSSLogger.getInstance().write("Error: " + e.getMessage());
			}
	...
	public void sendAMessage(Address a, String msg, byte messageType) {
		...
		if (data == null) {	
			CGSSLogger.getInstance().write("Aieeh ... encyption failed");
			return;
		}
...
//		AddressService.saveMessage(mess);
			...
			CGSSLogger.getInstance().write(e1.getMessage());
		}
	}
