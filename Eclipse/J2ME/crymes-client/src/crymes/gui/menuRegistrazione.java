...
public class menuRegistrazione extends List implements CommandListener {
	...
	private final CrymesMidlet gui;
...
//									   };
	...
	public menuRegistrazione(CrymesMidlet parent) {
...
//		SMSC = "+393804351160";
		...
		if (isEmulator())
			...
			SMSC = gui.getSettings().getSMSCNumber();
		}
	...
	public void completaRegistrazione(final CMessage challenge) {
		...
		// La challenge è stata inviata... lanciamo tutto il processo in una progress bar fino all'invio della
		// "challenge response"
		Display d = Display.getDisplay(gui);
		d.setCurrent(new ProgressScreen(this , "Elaborazione in corso", d) {
			...
			public void run() {
				...
				try {
					...
					new ModalAlert("Info", 
							   "La conferma è stata inviata.\nUn SMS cifrato confermerà l'attivazione del servizio.", 
							   AlertType.INFO, 
							   get_display(),
							   get_next(),
							   null,
					...
					gui.setTicketText("La conferma è stata inviata. Un SMS cifrato confermerà l'attivazione del servizio");
				}  
			}	
		}) ;
