...
public class GSMClient extends Thread implements IGenericClient {
	...
	public boolean send(crymes.util.wma.BinaryMessage msg) throws IOException  {
		...
		try {		
			...
			srv.sendMessage(out);
			sent = true;
			Logger.getInstance().write("Message sent to " + out.getRecipient());
		} catch (Exception e) {
			sent = false;
			Logger.getInstance().write("Error: impossible to send sms to " + out.getRecipient());
		}
		return sent;
...
//		srv.setAsyncRecvClass(CIncomingMessage.MessageClass.Read);
		...
		try {
			...
			Logger.getInstance().write("Error: impossible to set the receive mode");			
		}
	}
