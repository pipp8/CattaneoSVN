...
public abstract class MessageList extends List implements CommandListener {
	...
	public MessageList(CryptoSMSMidlet gui,String title) {
		super(title, Choice.IMPLICIT);
		this.gui = gui;

		this.back = new Command(gui.getResourceManager().getString(ResourceTokens.STRING_BACK), Command.BACK, 1);
		this.showText = new Command(gui.getResourceManager().getString(ResourceTokens.STRING_VIEW_MESSAGE), Command.OK,
				0);
		this.delete = new Command(gui.getResourceManager().getString(ResourceTokens.STRING_DELETE_MESSAGE), Command.OK,
				0);
