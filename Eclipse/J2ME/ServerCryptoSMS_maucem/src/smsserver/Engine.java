...
import smsserver.gui.MainFrame;
...
public class Engine  implements ISmsMessageListener{
	...
	public void sendSms (util.wma.BinaryMessage m){
	...
	public void notifyIncomingMessage(util.wma.BinaryMessage msg, boolean isEmulator)  {
		...
		final String gsm = gsmNumber;
			Runnable p = new Runnable() {
				public void run(){
					...
					try {
						Settings s=AddressService.saveSettings(n, gsm, CryptoHelper.generateKeyPairHelper());
						setSettings(s);
...
//								+ pers[i].getConns()[j].hasKey(), listImage(pers[i].getConns()[j].hasKey()));
	...
	public void PK_Register(CMessage m) {
		...
		if (m.getType() == MessageType.PK_REGISTER ) {
			...
			try { 
				...
				CMessage challenge = new CMessage(SMSC, a.getUrl(), MessageType.PK_CHALLENGE, data, new Date());
		...
		if (m.getType() == MessageType.PK_CHALLENGE_RESPONSE ) {
			...
			try {
				...
				if (equal){
					...
					String str = "Il processo di registrazione è avvenuto con successo. Da questo momento in poi tutte le comunicazioni saranno cifrate. CryptoTom";
	...
	public void sendAMessage(Address a, String msg) {
		String SMSC;
		if (a.getUrl().substring(0, 3).compareTo("+55") == 0)
			SMSC = CSettings.getNumberSMSC_WTK();
		else
		...
		byte[] data = CryptoHelper.encrypt(msg.getBytes(), a.getPublicKey());
		if (data == null) {	
			Logger.getInstance().write("Aieeh ... encyption failed");
			return;
		}
		
		CMessage mess = new CMessage(SMSC, a.getUrl(), MessageType.ENCRYPTED_TEXT_INBOX , data , new Date());
